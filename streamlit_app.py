...
.main-title { color: #0e3a66; font-weight: bold; text-align: center; margin-top: -40px; }
.caption-text { font-size: 11px; color: #6b7280; line-height: 1.4; margin-top: -8px; }
.info-card { padding: 12px !important; border-radius: 8px !important; font-size: 11px !important; line-height: 1.4 !important; height: 105px; display: flex; align-items: flex-start; }
...
with st.sidebar:
    ...
    st.markdown("---")
    lokasi_pilihan = st.selectbox("Pilih Lokasi", ["Raja Ampat", "Bunaken", "Lombok", "Pangandaran", "Kepulauan Derawan", "Karimunjawa", "Pulau Weh", "Bali", "Taman Laut Banda"])
...
# 4. KOORDINAT
daftar_koordinat = {
    "Raja Ampat": [-0.71, 130.65], "Bunaken": [1.68, 124.72], "Lombok": [-8.38, 116.03], 
    "Pangandaran": [-7.85, 108.50], "Bali": [-8.90, 115.15], 'Pulau Weh': [5.95, 95.30],
    'Karimunjawa': [-5.75, 110.45], 'Taman Laut Banda': [-4.60, 129.90], 'Kepulauan Derawan': [2.35, 118.35]
}
...
with st.container(border=True):
    ...
    folium.Marker(location=coords, popup=f"Titik Pengamatan: {lokasi_pilihan}", tooltip=f"Lokasi: {lokasi_pilihan}", icon=folium.Icon(color="blue", icon="info-sign")).add_to(m)
...
with c4:
    with st.container(border=True):
        ...
        st.markdown('<div style="display:flex; justify-content:space-between; font-size:11px; font-weight:600; color:#333333; margin-top:6px; margin-bottom:4px;"><span>26°C</span><span>28.5°C</span><span>31°C</span></div>', unsafe_allow_html=True)

st.markdown("<div style='margin-top: -10px;'></div>", unsafe_allow_html=True)
inf1, inf2, inf3, inf4 = st.columns(4)
with inf1:
    st.markdown('<div class="info-card info-blue"> ℹ️  Pemilihan tanggal minimum H-3 direkomendasikan karena transmisi data membutuhkan waktu penyediaan.</div>', unsafe_allow_html=True)
with inf2:
    if mode_peta == "Suhu Laut (SST)" and anomaly is not None:
        st.markdown('<div class="info-card info-green"> 💡  Perhitungan: Nilai SST aktual dikurangi rata-rata batas historis klimatologi bulanan (2010–2020).</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="info-card info-green"> 💡  Perhitungan: Menunggu data Suhu Permukaan Laut (SST) aktif untuk menghitung anomali.</div>', unsafe_allow_html=True)
with inf3:
    if anomaly is None:
        st.markdown('<div class="info-card info-orange"> 🔍  Kriteria: Menunggu input data SST. Status bleaching dihitung berdasarkan besaran deviasi nilai anomali.</div>', unsafe_allow_html=True)
    elif anomaly > 1:
        st.markdown('<div class="info-card info-orange"> 🔍  Kriteria: Anomali > 1°C. Suhu air laut melonjak tinggi, memicu stres termal akut dan pemutihan karang massal.</div>', unsafe_allow_html=True)
    elif anomaly > 0:
        st.markdown('<div class="info-card info-orange"> 🔍  Kriteria: Anomali 0°C s.d 1°C. Perairan terindikasi mengalami pemanasan ringan di atas rata-rata acuan lokal.</div>', unsafe_allow_html=True)
    else:
